...
public class InventoryCardServiceImpl implements InventoryCardService {
    ...
    @Override
    public Page<InventoryCardResponse> getUniqueCollectionCards(UUID ownerId, Pageable pageable) {

        Page<InventoryCard> uniqueEntries = inventoryCardRepository.findUniqueCardsByOwnerId(ownerId, pageable);

        return uniqueEntries.map(inventoryCardMapper::toResponse);
    }
